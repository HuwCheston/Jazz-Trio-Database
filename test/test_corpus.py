"""Tests for items in `\references\corpus.json`"""
...
class CorpusTest(unittest.TestCase):
    ...
    def test_track_metadata(
        self,
        required_cols: tuple = ('track_name', 'album_name', 'recording_year')
    ):
        """
        ...
        """
        self.assertEqual(
            self.df[list(required_cols)].isna().any().sum(), 0,
    ...
    def test_all_youtube_links_active(
        self,
        bad_pattern: str = '"playabilityStatus":{"status":"ERROR"'
    ):
        """
    ...
    def test_take_numbers_present(
        ...
        takes = [take for idx, grp in self.df.groupby(grps) for take in grp['track_take'].to_list() if len(grp) > 1]
        self.assertTrue(
            not any(math.isnan(take) for take in takes),
            msg='Some tracks with multiple takes do not have take numbers assigned.'
        )

    def test_timestamp_against_duration(
        self
    ):
        """
        ...
        def get_youtube_video_duration(
            li: str
        ) -> datetime:
            """
            ...
            try:
                with urllib.request.urlopen(search) as url:
                    response = json.loads(url.read())['items'][0]['contentDetails']['duration']
        ...
        self.assertIsNotNone(
            api_key,
            msg='YouTube API key not provided as environment variable'
        )
        for track in self.corpus:
            # Get the
            yt_links = [yt_link for yt_link in track['links']['external'] if 'youtube' in yt_link]
