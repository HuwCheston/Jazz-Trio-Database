...
class ItemMaker:
    ...
    def __init__(
            self, item: dict, index: int, output_filepath: str, **kwargs
    ):
        # Directories containing raw and processed (source-separated) audio, respectively
        self.raw_audio_loc = fr'{output_filepath}\raw\audio'
        self.separate_audio_loc = fr'{output_filepath}\processed\audio'
        ...
        self.in_file = fr'{self.raw_audio_loc}\{self.fname}.{self.file_fmt}'
        # Paths to all the source-separated audio files that we'll create (or load)
        self.out_files = [rf'{self.separate_audio_loc}\{self.fname}_{i}.{self.file_fmt}' for i in self.instrs]
        ...
        self.start, self.end = self._return_timestamp('start'), self._return_timestamp('end')
    ...
    def _get_item(
        ...
        self._logger_wrapper(f'... found {len(self.links)} valid link(s) to download from')
        ...
        for link in self.links:
            ...
            # If we get an error, continue on to the next link
                ...
                self._logger_wrapper(f'... error when downloading from {link} ({err}), trying next link')
                continue
            # If we've downloaded successfully, break out of the loop
            else:
                self._logger_wrapper(
                    f'... downloaded successfully from {link}'
                )
                break
        ...
        if not self._check_item_present_locally(self.in_file):
            raise DownloadError(f'Item {self.item["id"]} could not be downloaded, check input links are working')
    ...
    def _get_spleeter_cmd(
        ...
        return [
            ...
            '-o', f'{os.path.abspath(self.separate_audio_loc)}',  # Specifies the correct output directory
    ...
    def separate_audio(
        ...
        checks = [
            ...
            all(self._check_item_present_locally(fname) for fname in self.out_files),
            # Do all the source-separated items have the same duration as the original file?
            all([len(set(self._get_output_duration(fp) for fp in [self.in_file, out])) == 1 for out in self.out_files]),
            # Have we changed the timestamps for this item since the last time we built it?
            all(float(self.end - self.start) == float(self._get_output_duration(out)) for out in self.out_files),
    ...
    def finalize_output(
        ...
        for name, fpath in zip([*sorted(self.instrs), 'mix'], [*sorted(self.out_files), self.in_file]):
            self.item['output'][name] = os.path.abspath(fpath)
        self._logger_wrapper('... finished processing item')


@click.command()
@click.option('-i', 'input_filepath', type=click.Path(exists=True), default=r'..\..\references')
@click.option('-o', 'output_filepath', type=click.Path(exists=True), default='..\..\data')
def main(input_filepath, output_filepath):
    ...
    with open(input_filepath + '\corpus.json', "r+") as in_file:
        ...
        for index, item in enumerate(corpus, 1):
            ...
            made = ItemMaker(
                item=item, album_name_len=5, track_name_len=10,
                logger=logger, index=index, output_filepath=output_filepath
            )
    ...
    logger.info(f'dataset made in {round(time() - start)} secs, see output JSON at {os.path.abspath(output_fname)}')
